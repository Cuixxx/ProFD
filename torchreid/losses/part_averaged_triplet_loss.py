...
class PartAveragedTripletLoss(nn.Module):
    ...
    def __init__(self, margin=0.3, epsilon=1e-16, writer=None):
        ...
        self.hard_mine_triplet_loss = TripletLoss(margin=self.margin)
    ...
    def forward(self, part_based_embeddings, labels, parts_visibility=None):
        ...
        # if pairwise_dist.ndim == 3:
        #     loss = torch.stack([self.hard_mine_triplet_loss(item, labels)[0] for item in pairwise_dist], dim=0).mean(dim=0)
        #     return loss, 1, 1
        # return self.hard_mine_triplet_loss(pairwise_dist, labels)
