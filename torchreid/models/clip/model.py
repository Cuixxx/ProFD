...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        output, attn_map = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
        return output
...
class ResidualAttentionBlock_IVLP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False, i=0, design_details=None):
        ...
        self.n_cls_ctx = design_details["n_cls_ctx"]  # hyperparameter
        self.num_class = design_details["num_class"]
        self.n_ctx = design_details["n_ctx"]
        self.attn_mask = attn_mask
        if i != 0:
            self.add_prompt = add_prompt
            if self.add_prompt:

                cls_vectors = torch.empty(self.num_class, self.n_cls_ctx, d_model)
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            prefix = x[:self.n_ctx + 1, :, :]
            suffix = x[self.n_ctx + 1 + self.n_cls_ctx:, :, :]
            # Create/configure learnable tokens of this layer
            cls_ctx = cls_ctx.permute(1, 0, 2).half()
            # Add the learnable tokens of this layer with the input, replaced by previous
            # layer learnable tokens
            x = torch.cat([prefix, cls_ctx, suffix], dim=0)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0, text_layer=False, design_details=None):
        ...
        self.width = width
        self.layers = layers
        # if text_layer and design_details['text_encoder'] == 'deep_prompt':
        #     self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IVLP(width, heads, attn_mask, True, i,
        #                                                                  design_details) if prompts_needed > i
        #                                      else ResidualAttentionBlock_IVLP(width, heads, attn_mask, False, i, design_details)
...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 h_resolution: int, 
                 w_resolution: int, design_details=None
                 ):
        super().__init__()
        ...
        # prompt_till_layer_text = design_details['cls_depth']
        ...
        # self.f_tw = nn.Linear(embed_dim,1)
        # self.f_vw = nn.Linear(embed_dim,1)
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int, design_details=None):
