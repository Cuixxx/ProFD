...
class ImagePartBasedEngine1(Engine):
    ...
    def __init__(
        ...
        super(ImagePartBasedEngine1, self).__init__(config,
                                                   datamanager,
                                                   writer,
                                                   engine_state,
                                                   use_gpu=use_gpu,
    ...
    def forward_backward(self, data, epoch):
        ...
        text_embeddings = self.model(imgs, external_parts_masks=target_masks, get_text=True)
        ...
        loss, loss_summary = self.combine_losses_stage1(visibility_scores_dict,
                                                 embeddings_dict,
                                                 id_cls_scores_dict,
                                                 pids,
                                                 text_embeddings,
    ...
    def combine_losses_stage1(self, visibility_scores_dict, embeddings_dict, id_cls_scores_dict, pids, text_embeddings=None, pixels_cls_scores=None, target_masks=None, bpa_weight=0, reg_weight=10.0, ex_weight=10.0):
        # 1. Part prediction objective:
        ...
        if pixels_cls_scores is not None\
            ...
            loss_summary = bpa_loss_summary
        # 2. Calculate the L_SCL_image loss
        # norm_globl = embeddings_dict['globl']
        # norm_globl = norm_globl/norm_globl.norm(dim=-1, keepdim=True)
        # norm_zsfeatp = embeddings_dict['zsfeatp'][:, 0].cuda()
        # norm_zsfeatp = norm_zsfeatp/norm_zsfeatp.norm(dim=-1, keepdim=True)
        # loss_scl_image = 1-torch.einsum('ij,ij->i', norm_zsfeatp, norm_globl).mean()
        #
        # scl_loss_summary = {}
        # scl_loss_summary['feat'] = OrderedDict()
        # scl_loss_summary['feat']['c'] = loss_scl_image
        # loss += reg_weight * loss_scl_image
        # loss_summary = {**loss_summary, **scl_loss_summary}
        #3. feature-aware adaptation
        image_features = embeddings_dict['ex_parts']
        visibility_scores = visibility_scores_dict['ex_parts']
        image_features = image_features/image_features.norm(dim=-1, keepdim=True)
        text_features = text_embeddings
        text_features = text_features/text_features.norm(dim=-1, keepdim=True)
        mat = torch.matmul(image_features, text_features.permute(0, 2, 1))
        mat_T = mat.transpose(1, 2)
        label = torch.arange(text_features.shape[1]).repeat(mat.shape[0]).cuda()
        #occluded part features are not computed CE loss
        occluded_index = ~visibility_scores.flatten(0, 1)
        label[occluded_index] = -100

        loss_i2t = F.cross_entropy(mat.flatten(0, 1), label, ignore_index=-100)
        loss_t2i = F.cross_entropy(mat_T.flatten(0, 1), label, ignore_index=-100)
        ex_loss = loss_i2t + loss_t2i
        ex_loss_summary = {}
        ex_loss_summary['ex_parts'] = OrderedDict()
        ex_loss_summary['ex_parts']['c'] = ex_loss
        loss += bpa_weight * ex_loss * 0.5
        loss_summary = {**loss_summary, **ex_loss_summary}
        return loss, loss_summary
