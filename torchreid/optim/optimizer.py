...
def build_optimizer(
    ...
    if staged_lr:
        ...
        for key, value in model.named_parameters():
            ...
            if "ctx" in key:
                lr_ctx = lr * 70.0
                param_groups += [{"params": [value], "lr": lr_ctx, "weight_decay": weight_decay}]
                continue
            if "image_encoder" in key:
                if "bias" in key:
                    lr_bias = lr * 2.0
                    weight_decay_bias = 0.0001
                    param_groups += [{"params": [value], "lr": lr_bias, "weight_decay": weight_decay_bias}]
                else:
                    param_groups += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
            else:
                param_groups += [{"params": [value], "lr": lr*10.0, "weight_decay": weight_decay}]

            # if "image_encoder" in key:
            #     if "image_encoder.proj" in key:
            #         continue
            #     else:
            #         value.requires_grad_(False)
            #         continue
